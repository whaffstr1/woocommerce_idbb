	...
	public static function save( $post_id, $post ) {
		...
		if($post_id){
			...
			foreach ($term as $key2 => $value2) {
				foreach ($categories1 as $category0) {
					if($value2==$category0->term_id){
						$cat_lvl1=$value2;
						continue;
					}
				}
			...
			$key_find = array_search($cat_lvl1, $term);
			unset($term[$key_find]);
			...
			foreach ($term as $key2 => $value2) {
				foreach ($categories2 as $category0) {
					if($value2==$category0->term_id){
						$cat_lvl2=$value2;
						continue;
					}
			...
			if($cat_lvl2!='' || !empty($cat_lvl2)){
				$key_find = array_search($cat_lvl2, $term);
				unset($term[$key_find]);
				...
				foreach ($term as $key2 => $value2) {
					foreach ($categories3 as $category0) {
						if($value2==$category0->term_id){
							$cat_lvl3=$value2;
							continue;
						}
